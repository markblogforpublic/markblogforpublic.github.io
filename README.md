...
这个仓库采用 **双许可证（Dual-License）** 结构：

| 部分 | 许可证 | 说明 |
|------|--------|------|
| 源代码（HTML/CSS/JS） | **Apache 2.0** | 强制要求修改者声明改动，禁止以原作者名义发布 |
| 内容（文章/文字/图片） | **CC BY-NC-ND 4.0** | 禁止修改、禁止商业使用，可转发需署名 |
...
| 部分 | 许可证 | 作用 |
|------|--------|------|
| 源代码（HTML/CSS/JS） | **Apache 2.0** | 强制声明改动，商标条款保护原作者身份 |
| 内容（文章/文字/图片） | **CC BY-NC-ND 4.0** | 禁止修改、禁止商用，转载需署名 |

详见 [LICENSE.md](LICENSE.md)。

### 特别免责声明

1. **严禁挂名**：任何 Fork 或修改版不得使用原作者个人信息（ID、头像、域名、社交账号）发布。
2. **内容脱钩**：衍生版本的内容与原作者无关，原作者不对其负法律责任。
3. **禁止镜像**：未经允许，禁止将此站点镜像到涉及非法业务的服务器。
